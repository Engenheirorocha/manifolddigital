/* HVAC PRO - app.js
   ...
   Mantém somente a lógica que já existia no arquivo grande. */
...
function next(){current=(current+1)%cards.length;updateCarousel()}
function prev(){current=(current-1+cards.length)%cards.length;updateCarousel()}

function searchHome(){
  const value=document.getElementById("homeSearch").value.trim().toLowerCase();
  ...
  const found=map.find(item=>item.keys.some(k=>k.includes(value)||value.includes(k)));
  if(found){current=found.index;updateCarousel()}
}
...
function openScreen(id){
  ...
  if(id==='gases') document.getElementById('gases').scrollTop=0;
...
function renderGas(name){
  ...
  if(!gas){document.getElementById('gasInfo').innerHTML=`<h2>Não encontrado</h2><div class="info-row">Digite um gás válido.</div>`;return;}
  const ptRows=gas.pt.map(row=>`<tr><td>${row[0]}</td><td>${row[1]}</td></tr>`).join("");
...
function selectGas(name,el){document.querySelectorAll('.gas-chip').forEach(chip=>chip.classList.remove('active-gas'));el.classList.add('active-gas');document.getElementById('gasSearch').value=name;renderGas(name)}
function searchGas(){const value=document.getElementById('gasSearch').value.trim();if(value.length>=2) renderGas(value)}

function svgSplit(){return `<svg viewBox="0 0 100 100" fill="none"><rect x="15" y="25" width="70" height="32" rx="8" stroke="#ff3636" stroke-width="6"/><path d="M24 44H76" stroke="#ff3636" stroke-width="5" stroke-linecap="round"/><path d="M34 62C42 68 58 68 66 62" stroke="#ff3636" stroke-width="5" stroke-linecap="round"/></svg>`;}
function svgCassete(){return `<svg viewBox="0 0 100 100" fill="none"><rect x="22" y="18" width="56" height="56" rx="8" stroke="#ff3636" stroke-width="6"/><rect x="36" y="32" width="28" height="28" rx="5" stroke="#ff3636" stroke-width="5"/><path d="M50 20V32M50 60V72M24 46H36M64 46H76" stroke="#ff3636" stroke-width="5" stroke-linecap="round"/></svg>`;}
function svgJanela(){return `<svg viewBox="0 0 100 100" fill="none"><rect x="22" y="20" width="56" height="60" rx="8" stroke="#ff3636" stroke-width="6"/><path d="M30 38H70" stroke="#ff3636" stroke-width="5" stroke-linecap="round"/><path d="M34 52H66M34 63H66" stroke="#ff3636" stroke-width="5" stroke-linecap="round"/><circle cx="67" cy="70" r="4" fill="#ff3636"/></svg>`;}
function svgPisoTeto(){return `<svg viewBox="0 0 100 100" fill="none"><rect x="18" y="24" width="64" height="28" rx="7" stroke="#ff3636" stroke-width="6"/><path d="M28 42H72" stroke="#ff3636" stroke-width="5" stroke-linecap="round"/><path d="M30 58V76M50 58V76M70 58V76" stroke="#ff3636" stroke-width="5" stroke-linecap="round"/></svg>`;}

function activeCategory(){return errorCategories[catCurrent].name}
function activeBrand(){return (brandsByCategory[activeCategory()]||[])[brandCurrent]}
function activeModel(){return (modelsByBrand[activeBrand()]||[])[modelCurrent]||""}

function getCodes(){
  const key=activeBrand()+"|"+activeModel();
  return errorCodesByModel[key] || [
    {code:"E1",title:"Falha genérica",cause:"Código ainda não refinado para este modelo.",test:"Verificar alimentação, sensores, comunicação e placas.",solution:"Cadastrar diagnóstico específico na próxima etapa."},
    {code:"E3",title:"Ventilador / sensor",cause:"Possível falha no motor, sensor ou rotação.",test:"Verificar motor, capacitor, sensor Hall e placa.",solution:"Corrigir componente defeituoso."}
  ];
}
...
function nextCategory(){catCurrent=(catCurrent+1)%errorCategories.length;updateCategoryCarousel()}
function prevCategory(){catCurrent=(catCurrent-1+errorCategories.length)%errorCategories.length;updateCategoryCarousel()}

function searchErrorType(){
  const value=document.getElementById("errorTypeSearch").value.trim().toLowerCase();
  if(value.length<2)return;
  const index=errorCategories.findIndex(cat=>cat.search.some(term=>term.includes(value)||value.includes(term)));
  if(index>=0){catCurrent=index;updateCategoryCarousel()}
}
...
function searchBrand(){
  const value=document.getElementById("brandSearch").value.trim().toLowerCase();
  if(value.length<1)return;
  const brands=brandsByCategory[activeCategory()]||[];
  const index=brands.findIndex(brand=>{
    const b=brand.toLowerCase();
    return b.includes(value) || value.includes(b);
  });
  if(index>=0){brandCurrent=index;updateBrandCarousel()}
}
...
function searchModel(){
  const value=document.getElementById("modelSearch").value.trim().toLowerCase();
  const models=modelsByBrand[activeBrand()]||[];
  if(value.length<1){renderModelInfo();return;}
  const index=models.findIndex(model=>{
    const m=model.toLowerCase();
    return m.includes(value) || value.includes(m);
  });
  if(index>=0){modelCurrent=index;updateModelCarousel()}
  else{
    document.getElementById("modelInfo").innerHTML=`
      <h2>Modelo informado</h2>
      <div class="info-row"><span>Marca:</span><br>${activeBrand()}</div>
      <div class="info-row"><span>Modelo/Série digitado:</span><br>${document.getElementById("modelSearch").value}</div>
    `;
  }
}
...
function searchCode(){
  const value=document.getElementById("codeSearch").value.trim().toLowerCase();
  const codes=getCodes();
  if(value.length<1){renderCodeInfo();return;}
  const index=codes.findIndex(item=>{
    const c=item.code.toLowerCase();
    return c.includes(value) || value.includes(c);
  });
  if(index>=0){codeCurrent=index;updateCodeCarousel()}
}
